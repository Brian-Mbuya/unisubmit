...
/**
 * Session + formLogin chain for the server-rendered Thymeleaf app (every route except
 * {@code /api/v1/**}).
 * <p>
 * Ordered LAST on purpose: {@code ApiSecurityConfig} sits at {@code @Order(1)} and claims
 * {@code /api/v1/**} for the stateless JWT chain used by the Flutter client. This chain
 * declares no {@code securityMatcher}, so it catches everything else — including the
 * pre-existing {@code /api/academic/**}, {@code /api/users/**} and {@code /api/ai/**}
 * helpers, which are called from authenticated browser pages and must keep their session
...
public class SecurityConfig {
        ...
        http
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/css/**", "/js/**", "/images/**", "/favicon.ico", "/favicon.svg",
                        ...
                        "/login", "/register", "/forgot-password", "/reset-password", "/about", "/error", "/health").permitAll()
    ...
    /**
     * Exposed as a bean so {@code AuthApiController} (the JWT token endpoint) can
     * authenticate through the exact same path the browser login uses. Both chains
     * therefore share {@code CustomUserDetailsService}, which is where the brute-force
     * throttle, admission-number/staff-ID lookup, soft-delete and suspension checks live.
     * Behaviour for formLogin is unchanged — this is the provider Spring would have
     * built anyway, just made injectable.
     */
    @Bean
