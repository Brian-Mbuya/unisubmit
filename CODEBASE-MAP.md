...
- **Two security chains** since the Flutter pivot: session+formLogin for the web app,
  stateless JWT for `/api/v1/**` (mobile). **Read §7a before touching anything in `security/`.**
...
**Mobile API** (`controller/api/*`, **stateless JWT chain — see §7a**): `/api/v1/auth/login`
(POST, anonymous → {token,tokenType,expiresIn,role,displayName}; 401 bad_credentials /
403 suspended / 429 locked_out) · `/api/v1/auth/me`(GET, validates a stored token → identity;
the Flutter launch check). Role prefixes are reserved and enforced but NOT yet populated:
`/api/v1/student/**` `/api/v1/lecturer/**` `/api/v1/admin/**`. `controller/api/
ApiExceptionHandler` (@RestControllerAdvice, basePackages-scoped, HIGHEST_PRECEDENCE) keeps
these JSON instead of GlobalExceptionHandler's redirect-to-Referer.
**APIs (browser, session chain)**: `/api/ai-insights/{id}`(GET status poll) `/api/ai-insights/{id}/retry`(POST) ·
...
## 7a · Two security chains (added for the Flutter client — read before touching security)
- **`ApiSecurityConfig` @Order(1)**, `securityMatcher("/api/v1/**")`: STATELESS, CSRF off,
  bearer-token auth via `oauth2ResourceServer`. **`SecurityConfig` @Order(2)**, no matcher:
  everything else, unchanged — session + formLogin + CSRF. Spring uses the FIRST chain whose
  matcher hits, so the browser app is untouched.
- **The matcher is `/api/v1/**`, deliberately NOT `/api/**`** — the older `/api/academic/**`,
  `/api/users/**`, `/api/ai/**` helpers are called from authenticated browser PAGES and need
  their session cookie + CSRF token. The version prefix is the browser/mobile boundary.
- CSRF is off on the API chain ONLY because it is stateless + header-authenticated: browsers
  never attach an `Authorization` header cross-site, so there is nothing to forge. The
  template CSRF meta/hidden-input contract is untouched.
- `AuthApiController` authenticates through the SHARED `AuthenticationManager` bean (now
  exposed in `SecurityConfig`), so mobile login inherits `CustomUserDetailsService`'s
  brute-force throttle, admission-number/staff-ID lookup, soft-delete and suspension checks.
- **Suspension is enforced twice, by two filters**: `SuspensionEnforcementFilter` (session
  chain; tests `instanceof CustomUserDetails`, redirects to /login?suspended — now skips
  `/api/v1/`) and `security/jwt/JwtSuspensionFilter` (API chain; reads the `uid` claim,
  answers JSON 401). The second exists because the JWT principal is a `JwtAuthenticationToken`,
  so the first silently no-ops on API traffic. **Its `shouldRevoke` policy is an open owner
  decision** (per-request DB hit vs. trust-until-expiry vs. cached) — see its javadoc.
- `JwtTokenService` MUST pass an explicit `JwsHeader.with(MacAlgorithm.HS256)`; claims-only
  encoding defaults to RS256 and dies with "Failed to select a JWK signing key".
- Gotcha: `JwtSuspensionFilter` is a `@Component`, so a `FilterRegistrationBean(enabled=false)`
  in `ApiSecurityConfig` stops Boot ALSO auto-registering it as a plain servlet filter.

## 7 · Config truths (memorise these)
- **`API_JWT_SECRET`** (32+ chars) signs mobile tokens; unset → random key at boot with a WARN
  (works fine, but mobile clients log out on every redeploy — same as the in-memory sessions).
  `API_JWT_ACCESS_TTL` is an ISO-8601 duration, default `PT12H`.
